...
from pochi.runners.mock import Emit, Return, ScriptRunner, Wait
...
async def test_runner_contract_session_started_and_order() -> None:
    raw_completed: PochiEvent = ActionEvent(
    ...
    runner = ScriptRunner(script, engine=CLAUDE_ENGINE, resume_value="abc123")
...
async def test_runner_releases_lock_when_consumer_closes() -> None:
    gate = anyio.Event()
    runner = ScriptRunner([Wait(gate)], engine=CLAUDE_ENGINE, resume_value="sid")

    gen = cast(AsyncGenerator[PochiEvent, None], runner.run("hello", None))
    try:
        while True:
            evt = await anext(gen)
    ...
    gen2 = cast(
        AsyncGenerator[PochiEvent, None],
        runner.run("again", ResumeToken(engine=CLAUDE_ENGINE, value="sid")),
    )
    try:
        while True:
