...
from pochi.model import Action, ActionEvent, PochiEvent, ResumeToken, StartedEvent
from pochi.render import (
...
SAMPLE_EVENTS: list[PochiEvent] = [
    session_started("claude", "abc123", title="Claude"),
...
def test_progress_renderer_renders_progress_and_final() -> None:
    r = ExecProgressRenderer(
        max_actions=5, resume_formatter=_format_resume, engine="claude"
    )
    for evt in SAMPLE_EVENTS:
        r.note_event(evt)
...
def test_progress_renderer_clamps_actions_and_ignores_unknown() -> None:
    ...
    assert r.note_event(cast(PochiEvent, mystery)) is False


def test_progress_renderer_renders_commands_in_markdown() -> None:
    r = ExecProgressRenderer(max_actions=5, command_width=None, engine="claude")
    for i in (30, 31, 32):
        r.note_event(
            action_completed(
                f"item_{i}",
...
def test_progress_renderer_handles_duplicate_action_ids() -> None:
    r = ExecProgressRenderer(max_actions=5, engine="claude")
    events = [
...
def test_progress_renderer_deterministic_output() -> None:
    ...
    r1 = ExecProgressRenderer(max_actions=5, engine="claude")
    r2 = ExecProgressRenderer(max_actions=5, engine="claude")
...
def test_progress_renderer_ignores_missing_action_id() -> None:
    renderer = ExecProgressRenderer(engine="claude")
    resume = ResumeToken(engine="claude", value="abc")
    renderer.note_event(StartedEvent(engine="claude", resume=resume, title="Session"))
    ...
    assert header.startswith("working · claude · 0s")
